    ...
    while len(queue) > 0:
        ...
        if last_room == target:
            ...
            if last_room not in visited_rooms:
                ...
                for d in visited_map[str(last_room)]:
                    if visited_map[str(last_room)][d] not in visited_rooms:
                        new_path = path.copy()
                        new_path.append(d)
                        new_rooms_id = rooms_id.copy()
                        new_rooms_id.append(visited_map[str(last_room)][d])
                        rooms_id_list.append(new_rooms_id)

                        paths.append(new_path)
                        queue.append(visited_map[str(last_room)][d])
            else:
                pass
...
if __name__ == '__main__':
    ...
    room_search(visited_map, starting_room, 495)
    # sell(player)
